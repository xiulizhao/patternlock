(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.patternlock) {
      window.wwload.patternlock = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/patternLock/dist/patternlock.min", "css!libs/patternLock/dist/patternlock"], function (PatternLock) {
        window.wwload.patternlock = true;
        window.PatternLock = PatternLock;
        replace();
        fncallback();
      });
    } else if (window.wwload.patternlock === "wait") {
      setTimeout(function () {
  ...
  function patternLock_btn($ele) {
    var config = {};
    config.patternlockid = $ele.attr("data-patternlockid");
    config.matrix = JSON.parse($ele.attr("data-matrix"));
    config.margin = eval($ele.attr("data-margin"));
    config.radius = eval($ele.attr("data-radius"));
    config.patternVisible = eval($ele.attr("data-patternvisible"));
    config.lineOnMove = eval($ele.attr("data-lineonmove"));
    config.enableSetPattern = eval($ele.attr("data-enablesetpattern"))
    config.allowRepeat = eval($ele.attr("data-allowrepeat"));
    config.reset = $ele.attr("data--reset");

    $ele.find(".pattern_lock").attr("id", config.patternlockid)

    var lock = new PatternLock("#" + config.patternlockid, {
      matrix: config.matrix,
      margin: config.radius,               //定义模式圈的边距。
      radius: config.radius,                //定义图案圆的半径。
      patternVisible: config.patternVisible,       //在绘图时使图案可见.
      lineOnMove: config.lineOnMove,             //移动鼠标/手指时可以看到线条.
      enableSetPattern: config.enableSetPattern,        //如果设置为true，则允许动态设置模式
      allowRepeat: config.allowRepeat,              //如果为true则允许重复点。
      onDraw: function (pattern) {
        // Context is the pattern lock instance
        $.wwclass.helper.updateProp($ele, ("data-x-patternlock"), pattern);
        // setTimeout(function () {
        //     lock.reset();
        // }, 1000)
